...
import cn.alex.commonmodule.router.manager.ARouterPathManager.NATIVE_ITEM_VCC
...
fun getSchemaPathFromARouterPath(schemaPath: String?): String {
    ...
    val remoteUrlMatcher: Matcher? = when {
        ...
        schemaPath.startsWith(NATIVE_ITEM_VCC) -> schemaPath.matchVcc()
        schemaPath.startsWith(NATIVE_ITEM_FLUTTER) -> schemaPath.matchFlutter()
        else -> null
    }

    if (remoteUrlMatcher?.find() == true) {
        val schemaData = ARouterPathManager.items.find {
            // 分析本地 ARouter 路径，找到其 Path 内容，与服务端返回的路径比较
 ...
 * 解析 vcczxh 后面的内容
 */
fun String.matchVcc(): Matcher?{
    return Pattern.compile("(vcczxh)://([^?]+)\\??(.+)?").matcher(this)
}
 ...
 * 路径包括：xyqb://、/login/..
 */
fun aRouterNavigation(navUrl: String?, params: Map<String, String>? = null) {
    if (!navUrl.hasValue()) return
    ...
    if (postCard.type == RouteType.FRAGMENT) {
        ...
        if (!frag.hasValue() || frag!!.context == null) return
        val bundle = Bundle()
        params.value {
            for (p in it) {
        ...
        ContainerActivity.startWithFragName(frag.context!!, frag.javaClass.name, bundle)
    } else if (postCard.type == RouteType.ACTIVITY) {
        params.value {
            for (p in it) {
